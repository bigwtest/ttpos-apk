...
import 'package:pos/components/sub_screen/main.dart';
import 'package:pos/controllers/goserver/service.dart';
...
// import 'package:pos/utils/go.dart';

void mainEntry() async {
  WidgetsFlutterBinding.ensureInitialized();

  // final goServer = GoServer();
  // final dartServer = server.Server();
  ...
  appRunner() async {
    ...
    // NOTE: 启动 Dart 的 Http 服务
    // runZonedGuarded(
    //   () async => dartServer.start(),
    //   (error, stackTrace) {
    //     logger.severe(error);
    ...
    // NOTE: 启动 Go 打包成 so 的 Http 服务
    // runZonedGuarded(
    //   () async => goServer.start(),
    //   (error, stackTrace) {
    //     logger.severe(error);
    //     logger.severe(stackTrace);
    //   },
    ...
    if (!kIsWeb && Platform.isAndroid) {
      await GoServerService.startServer();
    }
...
void main() {
  final defaultRouteName = ui.PlatformDispatcher.instance.defaultRouteName;
  debugPrint('defaultRouteName: $defaultRouteName');
  if (defaultRouteName == "subMain") {
    debugPrint('defaultRouteName: $defaultRouteName');
    subScreenEntry();
  } else {
    mainEntry();
