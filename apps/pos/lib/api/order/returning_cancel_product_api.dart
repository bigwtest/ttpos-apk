...
/// 订单商品退菜
class OrderReturningCancelProductApi {
  final API _api = Get.find<APIController>().api;
  final Log _logger = Log(appName: 'OrderReturningCancelProductApi');
  ...
  Future<ResponseCart?> instant(RequestOrderOptProduct req, {ExtraRequestOptions? options}) async {
    try {
      final response = await _api.postWithRequestOptions(
        APIPath.instantOrderCartCancelReturning.cashierPath,
        data: req.toJson(),
  ...
  Future<ResponseCart?> desk(RequestOrderOptProduct req, {ExtraRequestOptions? options}) async {
    try {
      final response = await _api.postWithRequestOptions(
        APIPath.deskOrderCartCancelReturning.cashierPath,
        data: req.toJson(),
        requestOptions: options,
      );
      if (response.code.success) {
