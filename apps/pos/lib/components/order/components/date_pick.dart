...
// Project imports:
import 'package:pos/components/order/components/select_multiple_components.dart';
...
class DatePick extends StatefulWidget {
  ...
  final List<DateTime?> initialDialogCalendarPickerValue;
  ...
  const DatePick({
    ...
    this.onChanged,
    this.onDateChanged,
    this.initialDialogCalendarPickerValue = const [],
  });
...
class _DatePickState extends State<DatePick> {
  ...
  void initState() {
    ...
    dialogCalendarPickerValue = widget.initialDialogCalendarPickerValue;
  }

  @override
  void didUpdateWidget(DatePick oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.initialValue != oldWidget.initialValue) {
      value = widget.initialValue;
    }
    if (widget.initialDialogCalendarPickerValue != oldWidget.initialDialogCalendarPickerValue) {
      dialogCalendarPickerValue = widget.initialDialogCalendarPickerValue;
    }
  }
  ...
  Future<List<DateTime>?> _selectDateTime(BuildContext context, String type) async {
    ...
    return await showOmniDateTimeRangePicker(
      ...
      defaultTab: dialogCalendarPickerValue.isNotEmpty && type == 'end' ? DefaultTab.end : DefaultTab.start,
      ...
      child: ClipRRect(
        ...
        child: Flex(
          ...
          children: [
                  ...
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Text(
                        ...
                        dialogCalendarPickerValue.isEmpty
                            ? '开始时间'.tr
                            : _removeZero(
                                dialogCalendarPickerValue[0].toString(),
                              ).toString(),
                        style: TextStyle(
                          color: dialogCalendarPickerValue.isNotEmpty
                  ...
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Text(
                        ...
                        dialogCalendarPickerValue.isEmpty
                            ? '结束时间'.tr
                            : dialogCalendarPickerValue.length > 1
                                ? _removeZero(
                                    dialogCalendarPickerValue[1].toString(),
                                  ).toString()
                                : _removeZero(
                                    dialogCalendarPickerValue[0].toString(),
                                  ).toString(),
            ...
            if (dialogCalendarPickerValue.isEmpty)
              IconButton(
            ...
            if (dialogCalendarPickerValue.isNotEmpty)
              IconButton(
                padding: EdgeInsets.zero,
                iconSize: 14.0.scaleWidth,
                icon: Icon(
