...
class InitBinding extends Bindings {
  ...
  void dependencies() {
    ...
    /// 语言-副屏
    Get.put<LanguageSecondController>(
    ...
    Get.put<APIController>(
      APIController(
        ...
        onPrint: (RespPrinterData? data) {
          try {
            if (data != null) {
              PrintController.sendPrint(data);
            }
          } catch (error, stackTrace) {
            debugPrint('onPrint error: $error');
            debugPrintStack(stackTrace: stackTrace);
          }
        },
        onOrderMethodDisabled: () async {
          try {
            if (Get.isRegistered<BaseInfoController>()) {
              await Get.find<BaseInfoController>().loadBaseInfoFromAPI();
            }
          } catch (error, stackTrace) {
            debugPrint('onOrderMethodDisabled error: $error');
            debugPrintStack(stackTrace: stackTrace);
          }
        },
        isSameToRedirectToLoginWhenOrderMethodDisabled: false,
        isShowExpired: true,
      ),
      permanent: true,
    );
