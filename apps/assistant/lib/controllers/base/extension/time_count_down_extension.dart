...
class TimerMonitor {
  ...
  void _updateTimerCount(bool isIncrement) {
    ...
    debugPrint('TimerMonitor 当前活跃的定时器数量: ${activeTimers.value} ${isIncrement ? '↑' : '↓'}');
  }
  ...
  void _onTimerTick(Timer timer) {
    ...
    switch (mode) {
      case TimerMode.countdown:
        seconds.value--;
        if (seconds.value <= 0) cleanupTimer();
        break;
...
extension TimeCountDownExtension on int {
  ...
  String get timeDown {
    try {
      ...
      debugPrint('timeDown error: [$this], error: $error\n$stackTrace');
      return '00 : 00';
    }
  }
  ...
  String get timeUp {
    try {
      ...
      debugPrint('timeUp error: [$this], error: $error\n$stackTrace');
      return '00 : 00';
    }
  }
}
