...
import 'package:assistant/model/response/auth/base_info.dart';
...
part 'mixin/assistant_mixin.dart';
part 'mixin/assistant_staff_mixin.dart';
...
class BaseInfoController extends GetxController
  ...
  void loadBaseInfo({bool fromStorage = false, bool fromAPI = true}) async {
    ...
    if (fromAPI) {
      ...
      if (!Get.isRegistered<SharedPreferencesWithCache>()) return;
      final sharedPreferencesWithCache = Get.find<SharedPreferencesWithCache>();
      final isSelectCashier = sharedPreferencesWithCache.getInt(StorageKey.authIsSelectCashier.asString);
      if (isSelectCashier == null || isSelectCashier != 1) return;
